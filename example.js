...
function exportBatch() {
    ...
    var exportData = { r: encodedBatch, d: encodeDict(dict) };
    //console.log("Unencoded export data", batch)
    //console.log("Encoded export data", exportData)
    var jsOrig = JSON.stringify(origBatch);
    var jsEncoded = JSON.stringify(exportData);
    batch = [];
    var end = performance.now();
    console.log("Original JSON len:", jsOrig.length, "bytes,", "Encoded JSON len:", jsEncoded.length, "bytes");
    console.log("Encoding time: ".concat(end - start, " ms"));
}
function encodeObject(obj, dict) {
    var encoded = {};
    for (var key in obj) {
        var keyRef = encodeVal(key, dict);
        var val = obj[key];
...
function encodeDict(dict) {
    var d = [];
    for (var key in dict.strings) {
        var ref = dict.strings[key];
        d[ref] = key;
    }
    return d;
}
